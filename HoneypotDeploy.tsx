...
export default function HoneypotDeploy({ onScoreChange }: HoneypotDeployProps) {
  ...
  const attackerIdCounterRef = useRef(0);
  const logIdCounterRef = useRef(0);
  const onScoreChangeRef = useRef(onScoreChange);
  // Track attacker clear timeouts for cleanup
  const attackerTimeoutsRef = useRef<ReturnType<typeof setTimeout>[]>([]);
  ...
  // Cleanup all attacker timeouts on unmount or game reset
  const clearAllAttackerTimeouts = useCallback(() => {
    attackerTimeoutsRef.current.forEach(clearTimeout);
  ...
  const spawnAttacker = useCallback(() => {
    ...
    const honeypotNodes = currentNodes.filter((n) => n.type === 'honeypot');
    const realNodes = currentNodes.filter((n) => n.type === 'real');
    ...
    const attackerId = attackerIdCounterRef.current++;
    ...
    if (targetNode.type === 'honeypot') {
      ...
      setScore((prev) => {
        ...
        onScoreChangeRef.current(Math.min(500, newScore));
        return newScore;
      });
      setTrappedCount((prev) => prev + 1);
    }

    // Clear attacker after delay - track timeout for cleanup
    ...
    attackerTimeoutsRef.current.push(timeoutId);
    // Prune old timeout refs after they fire
    setTimeout(() => {
      attackerTimeoutsRef.current = attackerTimeoutsRef.current.filter(id => id !== timeoutId);
    }, 4100);
  }, []);
  ...
  const startGame = () => {
    ...
    setTrappedCount(0);
    setActionCount(0);
    attackerIdCounterRef.current = 0;
    ...
    // Clear any stale timeouts from previous game
    clearAllAttackerTimeouts();
  };
