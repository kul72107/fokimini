...
export default function HashCracker({ onScoreChange }: Props) {
  ...
  // Use proper browser-compatible types instead of NodeJS.Timeout
  const processRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const gearRef = useRef<ReturnType<typeof setInterval> | null>(null);
  // Refs to hold latest values inside interval callbacks to avoid stale closures
  const challengeRef = useRef(selectedChallenge);
  const wordlistRef = useRef(selectedWordlist);
  const speedRef = useRef(speed);
  const showProcessRef = useRef(showProcess);

  // Keep refs in sync with state
  useEffect(() => { challengeRef.current = selectedChallenge; }, [selectedChallenge]);
  useEffect(() => { wordlistRef.current = selectedWordlist; }, [selectedWordlist]);
  useEffect(() => { speedRef.current = speed; }, [speed]);
  useEffect(() => { showProcessRef.current = showProcess; }, [showProcess]);
  ...
  const runCrack = useCallback(() => {
    ...
    const targetHash = challengeRef.current ? challengeRef.current.hash : hashInput;
    ...
    const wordlist = WORDLISTS[wordlistRef.current] || WORDLISTS.common;
    const totalSteps = challengeRef.current ? wordlist.length : 100;
    ...
    const process = setInterval(() => {
      ...
      setAttempts(step * Math.floor(currentSpeed / 10 + 1));
      ...
      if (currentChallenge && step >= totalSteps) {
        clearInterval(process);
        processRef.current = null;
        stopGearAnimation();
        const elapsed = ((Date.now() - startTime) / 1000).toFixed(2);
        setCrackTime(elapsed + 's');
        setFoundPassword(currentChallenge.plaintext);
        setHashesCracked(prev => prev + 1);
        setCrackProgress(100);
        const points = currentChallenge.difficulty === 'Easy' ? 100 : currentChallenge.difficulty === 'Medium' ? 150 : currentChallenge.difficulty === 'Hard' ? 200 : currentChallenge.difficulty === 'Expert' ? 250 : 300;
        addScore(points);
        setIsCracking(false);
        setShowStrength(true);
      } else if (!currentChallenge && step >= totalSteps) {
        clearInterval(process);
        processRef.current = null;
        stopGearAnimation();
    ...
    }, 1200 - speedRef.current * 10);

    processRef.current = process;
  }, [hashInput, startGearAnimation, stopGearAnimation, addScore]);
