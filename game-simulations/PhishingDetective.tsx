...
export default function PhishingDetective({ onScoreChange }: PhishingDetectiveProps) {
  ...
  const handleElementClick = (elementId: string) => {
    ...
    setScore(newScore);
    const percent = Math.min(100, Math.max(0, (newScore / totalScore) * 100));
    onScoreChange(percent);
  ...
  // Count elements found for the CURRENT email only
  const currentFoundCount = currentEmail.suspiciousElements.length > 0
    ? foundElements.filter((f) => currentEmail.suspiciousElements.some((e) => e.id === f)).length
    : 0;
  const allFound = currentEmail.suspiciousElements.length > 0
    ? currentFoundCount === currentEmail.suspiciousElements.length
    : true;
          ...
          >
            ...
            <div className="border-t-[3px] border-black p-4 bg-purple-pale flex items-center justify-between">
              ...
              <div className="font-nunito text-xs text-purple-light">
                Found: {currentFoundCount} / {currentEmail.suspiciousElements.length}
              </div>
            </div>
          </motion.div>
        </AnimatePresence>
              ...
              <button
                onClick={() => {
                  ...
                  if (allFound) {
                    ...
                    setScore(bonus);
                    const percent = Math.min(100, Math.max(0, (bonus / totalScore) * 100));
                    onScoreChange(percent);
                  }
                }}
