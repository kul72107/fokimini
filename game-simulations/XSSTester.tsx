...
export default function XSSTester({ onScoreChange }: Props) {
  ...
  // Track which XSS payloads have been scored to prevent duplicates
  const scoredXssRef = useRef<Set<string>>(new Set());
  // Track if sanitized score has been awarded to prevent toggle exploit
  const sanitizedAwardedRef = useRef(false);
  ...
  const runTest = useCallback((payload: XSSPayload) => {
    ...
    if (cookieTimeoutRef.current) clearTimeout(cookieTimeoutRef.current);
    ...
    testTimeoutRef.current = setTimeout(() => {
      ...
      if (isMalicious) {
        // Only award score for newly discovered XSS
        if (!scoredXssRef.current.has(payload.id)) {
          scoredXssRef.current.add(payload.id);
              ...
              >
                ...
                {xssDetected && (
                  <>
                    ...
                    <button
                      onClick={() => {
                        setShowSanitized(!showSanitized);
                        // Only award points once for viewing sanitized version
                        if (!showSanitized && !sanitizedAwardedRef.current) {
                          sanitizedAwardedRef.current = true;
                          addScore(20);
                        }
                      }}
