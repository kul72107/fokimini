...
export default function IDSAlert({ onScoreChange }: IDSAlertProps) {
  ...
  // Keep scoreRef in sync with score state for use in timeouts
  useEffect(() => {
  ...
  const handleAction = (action: string) => {
    ...
    if (isCorrectDevice && isCorrectAction) {
      ...
      // FIX: Use functional update for score to avoid stale closure
      setScore((prev) => {
    ...
    setTimeout(() => {
      if (currentAlertIndex < levelAlerts.length - 1) {
        ...
        setGameActive(false);
        setShowStart(true);
        // FIX: Use scoreRef to get the latest score value for the final message
        setMessage(`Game Complete! Final Score: ${scoreRef.current}`);
      }
    }, 2500);
  };
