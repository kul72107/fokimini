...
export default function LoadBalancer({ onScoreChange }: LoadBalancerProps) {
  ...
  const overloadTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    if (overloadWarning) {
      ...
      overloadTimeoutRef.current = setTimeout(() => setOverloadWarning(''), 2000);
      return () => {
        if (overloadTimeoutRef.current) clearTimeout(overloadTimeoutRef.current);
      };
    }
  ...
  }, [servers.map((s) => s.connections).join(','), gameActive, gameComplete]);

  const handlePacketClick = (packet: Packet) => {
    if (!gameActive || packet.serverId !== null || gameComplete) return;
    ...
    // FIX: Add win condition when level 4 completes 10 packets
    if (packetsProcessed + 1 >= 10) {
      if (level < 4) {
        ...
        // Level 4 complete - game won!
        setGameComplete(true);
        setMessage(`All levels complete! Final Score: ${score}/100`);
      }
    }
  };
      ...
      <AnimatePresence>
        {gameComplete && (
          <motion.div
            ...
            animate={{ scale: 1, opacity: 1 }}
            exit={{ scale: 0, opacity: 0 }}
            className="w-full bg-green-success border-[3px] border-black rounded-xl p-3 flex items-center justify-center gap-2"
          >
            <Check size={24} strokeWidth={3} className="text-white" />
            <span className="font-nunito text-sm font-bold text-white">
              All Levels Complete! Final Score: {score}/100
            </span>
          </motion.div>
        )}
      </AnimatePresence>
