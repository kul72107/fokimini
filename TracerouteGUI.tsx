...
export default function TracerouteGUI({ onScoreChange }: Props) {
  ...
  // Use refs to track latest values in async callbacks without effect restarts
  const totalScoreRef = useRef(totalScore);
  const isScanningRef = useRef(isScanning);
  const currentHopRef = useRef(currentHop);
  const hopsRef = useRef(TARGETS[selectedTarget].hops);
  ...
  useEffect(() => { hopsRef.current = TARGETS[selectedTarget].hops; }, [selectedTarget]);
  ...
  const startTraceroute = useCallback(() => {
    ...
    setCompleted(false);
    setSelectedHop(null);
    // Reset score for new traceroute session
    setTotalScore(0);
  ...
  useEffect(() => {
    ...
    const hopList = hopsRef.current;

    if (currentHop < hopList.length - 1) {
      const timer = setTimeout(() => {
        const nextHop = currentHop + 1;
        setCurrentHop(nextHop);

        // Update score for each discovered hop using ref to avoid stale closure
        if (hopList[nextHop].status !== 'timeout') {
          setTotalScore(prev => {
        ...
        setPacketPos({ x: hopList[nextHop].x, y: hopList[nextHop].y });
      }, 1200);
