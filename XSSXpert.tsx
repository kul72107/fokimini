...
export default function XSSXpert({ onScoreChange }: Props) {
  ...
  // Use ref to track correct count and timeout to avoid stale closures
  ...
  const handleAction = (action: ActionType) => {
    ...
    // Update correct count ref immediately to avoid stale closure in timeout
    if (correct) {
    ...
    actionTimeoutRef.current = setTimeout(() => {
      if (currentIndex + 1 >= comments.length) {
        // Use ref for correct count instead of stale processedComments state
        const allCorrect = correctCountRef.current === comments.length;
