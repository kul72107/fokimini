  ...
  const finishRequest = (websiteId: string, cached: boolean) => {
    ...
    if (requestCount + 1 >= 6 && level < 3) {
      setTimeout(() => {
        ...
        setCache([]);
        setRequestCount(0);
        setMessage(`Level ${level + 1}: ${levelTitles[level]}! Try requesting more sites.`);
      }, 2000);
    }
  };
          ...
          <div className="w-full flex gap-2 justify-center flex-wrap">
            ...
            <button
              onClick={() => {
                ...
                setMessage(contentFilterOn ? 'Content filter disabled.' : 'Content filter enabled! Reddit is now blocked.');
              }}
