...
const SOCIAL_ICONS: Record<string, React.ReactNode> = {
  Twitter: <Twitter size={14} strokeWidth={3} />,
  GitHub: <Github size={14} strokeWidth={3} />,
  LinkedIn: <Linkedin size={14} strokeWidth={3} />,
  Instagram: <Instagram size={14} strokeWidth={3} />,
  YouTube: <Youtube size={14} strokeWidth={3} />,
...
const TARGETS: TargetProfile[] = [
  {
    ...
    dataPoints: [
      ...
      { category: 'domain', label: 'Portfolio', value: 'johndoe-portfolio.github.io', icon: <Github size={14} strokeWidth={3} />, color: '#7C3AED' },
...
export default function OSINTScanner({ onScoreChange }: Props) {
  ...
  useEffect(() => {
    ...
    const advanceStep = () => {
      if (currentStep >= SCAN_STEPS.length) {
        ...
        setScanCount(c => c + 1);
        const newScore = totalScore + allData.length * 20 + 10;
  ...
  }, [isScanning, target, totalScore, onScoreChange]);
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="font-nunito text-[10px] text-blue-700 mt-1">
                  Every photo, social post, and public record leaves a digital footprint. Security researchers use OSINT to understand what information attackers can find about a target. Each data point found is worth +20 points.
                </p>
              </div>
            </div>
          </motion.div>
