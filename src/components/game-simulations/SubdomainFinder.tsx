...
export default function SubdomainFinder({ onScoreChange }: Props) {
  ...
  useEffect(() => {
    ...
    const wordlist = currentMode.wordlist;
    const targetSubs = target.subdomains;
    const totalChecks = wordlist.length;
    let currentIdx = 0;

    const interval = setInterval(() => {
      if (currentIdx >= totalChecks) {
        clearInterval(interval);
        setIsScanning(false);
        setScanComplete(true);
        setScanCount(c => c + 1);
        const foundCount = foundSubs.length;
        const newScore = totalScore + foundCount * 10 + 5;
      ...
      setScanningSub(word);
      setScanProgress(Math.round(((currentIdx + 1) / totalChecks) * 100));
  ...
  }, [isScanning, target, currentMode, totalScore, onScoreChange, foundSubs.length]);
