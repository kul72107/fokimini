...
export default function PacketTracer({ onScoreChange }: Props) {
  ...
  const nextHop = useCallback(() => {
    ...
    setCurrentHop(prev => {
      const next = prev + 1;
      if (next >= journey.hops.length) {
        setIsPlaying(false);
        setAutoPlay(false);
        if (journey.successful) {
          setTotalScore(s => {
            const newScore = s + 15;
  ...
  }, [journey, onScoreChange]);

  useEffect(() => {
    if (!autoPlay || !isPlaying) return;
    const interval = setInterval(() => {
      if (journey && currentHop >= journey.hops.length - 1) {
        setAutoPlay(false);
        setIsPlaying(false);
        return;
      }
  ...
  }, [autoPlay, isPlaying, currentHop, journey, nextHop]);
  ...
  const addLog = (entry: string) => {
    setLogEntries(prev => [...prev.slice(-20), entry]);
  };
    ...
    <div className="flex flex-col gap-3 p-4 max-w-5xl mx-auto">
      ...
      {journey && (
        <div className="flex items-center justify-center gap-2">
          ...
          <button
            onClick={nextHop}
            disabled={!isPlaying || currentHop >= journey.hops.length - 1}
          ...
          <div className={`bg-purple-darker rounded-xl border-[3px] border-black p-2 font-mono text-[10px] overflow-y-auto ${showWireshark ? 'max-h-48' : 'max-h-24'}`}>
            ...
            {journey && currentHop >= journey.hops.length - 1 && (
              <div className={`py-1 font-bold ${journey.successful ? 'text-green-success' : 'text-red-alert'}`}>
                --- {journey.successful ? 'Packet delivered! +' + (journey.successful ? '15 pts' : '0 pts') : 'Packet dropped!'} ---
              </div>
            )}
          </div>
        </div>
