...
export default function SocialProfiler({ onScoreChange }: Props) {
  ...
  const searchSourceAction = (sourceId: string) => {
    ...
    setTimeout(() => {
      ...
      const found = dataPoints.filter(dp => dp.source === sourceLabel && !dp.found);
      ...
      if (found.length > 0) {
        ...
        setDataFound(prev => prev + found.length);
        const riskPoints = found.filter(f => f.risk === 'high').length;
        if (riskPoints > 0) {
          setRisksFound(prev => prev + riskPoints);
          addScore(15 * found.length + 50 * riskPoints);
        } else {
          addScore(15 * found.length);
        }
      }
  ...
  const selectProfile = (index: number) => {
    ...
    setDataPoints(TARGET_PROFILES[index].dataPoints);
    setPhase('idle');
    setDataFound(0);
    setRisksFound(0);
