...
import {
  ...
  ShieldAlert, Info, ChevronDown, Lock, Unlock, Eye, EyeOff,
  ...
  const handleAttack = () => {
    ...
    const interval = setInterval(() => {
      ...
      for (let i = 0; i < Math.min(plaintext.length, crib.length); i++) {
        decrypted.push(String.fromCharCode(plaintext.charCodeAt(i) ^ crib.charCodeAt(i)));
      }
      const partial = decrypted.join('');

      // Check if partial could be part of the key
      if (partial.toLowerCase().includes(key.toLowerCase().slice(0, 2)) ||
          key.toLowerCase().includes(partial.toLowerCase().slice(0, 2))) {
        clearInterval(interval);
        setAttackInProgress(false);
