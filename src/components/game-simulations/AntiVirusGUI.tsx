...
export default function AntiVirusGUI({ onScoreChange }: AntiVirusGUIProps) {
  ...
  useEffect(() => {
    ...
    if (currentIndex >= totalFiles) {
      ...
      setEducationalTip(`✅ Scan complete! Found ${threatsFound} threats. Quarantine them to keep your system safe!`);
  ...
  }, [scanning, currentIndex, totalFiles, scanType, threatsFound]);
  ...
  useEffect(() => {
    ...
    setScore(newTotal);
    onScoreChange(Math.min(500, newTotal));
  }, [files, onScoreChange]);
