...
export default function LogAnalyzer({ onScoreChange }: { onScoreChange: (score: number) => void }) {
  ...
  const filteredLogs = useMemo(() => {
    ...
    return logs;
  }, [c, filterType, searchQuery, showSuspiciousOnly, sortByTime]);

  // Stats
  const flaggedCount = flaggedLogs.size;
  const suspiciousCount = c.logs.filter((l) => l.suspicious).length;
  const progress = Math.min(100, Math.round((flaggedCount / suspiciousCount) * 100));
  ...
  const handleSolve = useCallback(() => {
    ...
    const correct = selectedAttack === c.attackType;
    const score = correct ? 100 : Math.round((flaggedCount / suspiciousCount) * 50);
  ...
  }, [selectedAttack, c.attackType, flaggedCount, suspiciousCount, onScoreChange, currentCase]);
    ...
    <div className="flex flex-col items-center gap-3 p-4">
      ...
      <div className="w-full max-w-2xl flex items-center justify-between bg-purple-dark rounded-xl border-[3px] border-black px-4 py-2">
        <div className="flex items-center gap-2">
          ...
          <span className="font-fredoka text-sm font-bold text-white">Case {c.id}: {c.name}</span>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="w-full max-w-2xl bg-blue-info rounded-xl border-[3px] border-black px-3 py-1.5 flex items-center gap-2">
        ...
        <p className="font-nunito text-xs text-white font-semibold">{c.description}</p>
      </div>
      ...
      <div className="w-full max-w-2xl bg-white rounded-2xl border-4 border-black p-3" style={{ boxShadow: '5px 5px 0px 0px #000' }}>
        ...
        <div className="flex gap-1.5">
          ...
          <button
            onClick={() => {
              const suspicious = c.logs.filter((l) => l.suspicious);
          ...
          <button
            onClick={() => {
              ...
              if (!showHint) setHintIdx((prev) => Math.min(prev, c.hints.length - 1));
            }}
            ...
            >
              <div className="mt-2 bg-yellow-accent rounded-lg border-[2px] border-black p-2 flex items-start gap-2">
                ...
                <div className="flex-1">
                  <p className="font-nunito text-[9px] text-black font-semibold">{c.hints[hintIdx]}</p>
          ...
          {showResult && (
            <motion.div
              ...
              className={`w-full max-w-2xl rounded-2xl border-4 border-black p-4 flex flex-col items-center gap-2 ${
                selectedAttack === c.attackType ? 'bg-green-success' : 'bg-red-alert'
              }`}
              style={{ boxShadow: '6px 6px 0px 0px #000' }}
            >
              {selectedAttack === c.attackType ? (
                <>
                  ...
                  <p className="font-nunito text-xs text-black text-center font-semibold">{c.attackExplanation}</p>
                  <button
                ...
                <>
                  ...
                  <p className="font-nunito text-xs text-white text-center">
                    This was actually a <strong>{ATTACK_OPTIONS.find((a) => a.type === c.attackType)?.label}</strong>.
                  </p>
                  <p className="font-nunito text-[10px] text-white/80 text-center">{c.attackExplanation}</p>
          ...
          >
            <div className="p-3">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="font-fredoka text-xs font-bold text-black">{c.learningTopic}</span>
              </div>
              <p className="font-nunito text-[10px] text-black font-semibold">{c.learningContent}</p>
              <button
