...
export default function KeyloggerSim({ onScoreChange, opsContext }: Props) {
  ...
  const captureKey = useCallback((key: string, isPasswordField = false) => {
    ...
    const newCapture: CapturedKey = {
      ...
      source: activeWindow,
      isPassword: isPasswordField && captureMode !== 'all',
    };
    ...
    if (isPasswordField && victimInput.length > 4) {
      ...
      if (match && Math.random() > 0.7) {
        match.captured = true;
        setFoundPassword(match);
        setPasswordsFound(prev => prev + 1);
        addScore(100);
  ...
  const handleKeyPress = (key: string) => {
    ...
    if (key === 'SPACE') {
      ...
      captureKey(key, victimInput.length > 6);
    }
  };

  const handleClipboardCopy = () => {
    if (!isKeyloggerActive || captureMode !== 'clipboard') return;
    const content = victimInput || 'copied text';
    setClipboardContent(content);
    setShowClipboardCapture(true);
    captureKey(`[CLIPBOARD: ${content.slice(0, 20)}]`);
    setTimeout(() => setShowClipboardCapture(false), 2000);
  };
          ...
          >
            ...
            <div className="bg-green-success/20 rounded-2xl border-4 border-green-success p-4">
              ...
              <ul className="space-y-1 text-xs font-nunito text-purple-dark">
                ...
                <li className="flex items-start gap-1">
                  <Check size={12} strokeWidth={3} className="text-green-success flex-shrink-0.5 mt-0.5" />
