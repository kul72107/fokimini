...
export default function CertChampion({ onScoreChange }: Props) {
  ...
  // Refs for timeouts to ensure cleanup on unmount
  ...
  const handleSlotClick = (fieldId: string) => {
    ...
    if (level.mode === 'fix') {
      if (fieldId === level.brokenField) {
        ...
        // Animate lock
        addTimeout(() => setLockState('closing'), 300);
        addTimeout(() => {
          setLockState('locked');
          setConfetti(true);
          ...
          addTimeout(() => setConfetti(false), 3000);
    ...
    if (correct) {
      ...
      // Check if all slots filled using functional state update
      setSlots(prevSlots => {
        const newSlots = { ...prevSlots, [fieldId]: piece };
        ...
        if (allFilled) {
          ...
          addTimeout(() => {
            ...
            // Use functional update to avoid stale closure
            setScore(prevTotal => {
              const newTotal = prevTotal + finalScore;
