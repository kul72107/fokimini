...
import {
  ...
  AlertTriangle, Eye, Sparkles, Lock, Globe,
...
export default function PhishingSim({ onScoreChange }: Props) {
  ...
  // Track if build score has been awarded to prevent infinite score exploit
  const buildScoreAwardedRef = useRef(false);
  ...
  const calculateBuildScore = useCallback(() => {
    ...
    setBuildScore(pts);
    // Only award score once, preventing infinite score exploit
    if (pts >= 30 && !buildScoreAwardedRef.current) {
      buildScoreAwardedRef.current = true;
      addScore(50);
    }
  }, [builder, addScore]);
  ...
  const reset = useCallback(() => {
    ...
    setBuildScore(0);
    buildScoreAwardedRef.current = false;
    onScoreChange(0);
  }, [onScoreChange]);
