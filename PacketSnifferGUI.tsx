...
export default function PacketSnifferGUI({ onScoreChange }: Props) {
  ...
  // Protocol bonus - award once when first hitting 5+ protocols
  useEffect(() => {
    if (protocolsIdentified.size >= 5 && !bonusAwardedRef.current) {
      bonusAwardedRef.current = true;
      const bonus = 50; // Fixed one-time bonus
      setTotalScore(prev => {
    ...
    // Reset bonus when protocols are cleared
    if (protocolsIdentified.size === 0) {
      bonusAwardedRef.current = false;
    }
  }, [protocolsIdentified.size, onScoreChange]);
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="font-nunito text-[10px] text-blue-700 mt-1">
                  Each packet has headers (like an envelope address) and payload (the actual message). Different protocols (HTTP, DNS, TCP) are like different languages. Each packet captured = +5 points. Identifying 5+ protocols = +50 bonus!
                </p>
              </div>
            </div>
          </motion.div>
