...
export default function SQLSafari({ onScoreChange }: Props) {
  ...
  // Use ref to track correct answers count for stale closure avoidance
  ...
  const startLevel = (lvl: LevelId) => {
    ...
    setMode('tutorial');
    onScoreChange(0);
    // resetLevel is called by the useEffect when currentLevel changes
  };
  ...
  const handleLevelComplete = (allCorrect: boolean) => {
