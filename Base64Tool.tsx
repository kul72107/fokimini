...
import {
  ...
  RotateCcw, ArrowRight, ArrowLeft, Lock, Unlock,
...
export default function Base64Tool({ onScoreChange }: Props) {
  ...
  // Track one-time score awards to prevent infinite score exploit
  const tableScoreAwardedRef = useRef(false);
  const learnScoreAwardedRef = useRef(false);
  const quizScoreAwardedRef = useRef(false);
  ...
  const performConversion = useCallback(() => {
    // Clear any existing animation timeout to prevent competing animations
    if (animRef.current) {
  ...
  const reset = useCallback(() => {
    ...
    tableScoreAwardedRef.current = false;
    learnScoreAwardedRef.current = false;
    quizScoreAwardedRef.current = false;
    onScoreChange(0);
  }, [onScoreChange]);
        ...
        <div className="lg:col-span-4">
          <button
            onClick={() => {
              ...
              // Only award points first time (prevent infinite score exploit)
              if (!showTable && !tableScoreAwardedRef.current) {
                tableScoreAwardedRef.current = true;
                addScore(10);
              }
            }}
        ...
        <div className="lg:col-span-4">
          <button
            onClick={() => {
              ...
              // Only award points first time (prevent infinite score exploit)
              if (!showLearn && !learnScoreAwardedRef.current) {
                learnScoreAwardedRef.current = true;
                addScore(10);
              }
            }}
            className="w-full mb-2 px-4 py-2 rounded-2xl border-[3px] border-black font-fredoka text-sm flex items-center justify-center gap-2 transition-transform hover:scale-[1.02] bg-purple-primary text-white"
                  ...
                  <div className={`rounded-xl border-[3px] p-2 mt-2 ${
                    ...
                    {!learnNotEncryption ? (
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => {
                            setLearnNotEncryption(true);
                            // Only award quiz points once
                            if (!quizScoreAwardedRef.current) {
                              quizScoreAwardedRef.current = true;
                              addScore(20);
                            }
                          }}
