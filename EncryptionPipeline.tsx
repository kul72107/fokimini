...
export default function EncryptionPipeline({ onScoreChange }: EncryptionPipelineProps) {
  ...
  // Refs for timeout cleanup and avoiding stale closures
  ...
  useEffect(() => {
    completedRef.current = completedPipelines;
  }, [completedPipelines]);
  ...
  const runPipeline = () => {
    ...
    // Clear any existing timeouts before starting new pipeline
    clearAllTimeouts();
    ...
    const t1 = setTimeout(() => {
      ...
      const t2 = setTimeout(() => {
        ...
        const t3 = setTimeout(() => {
          let decrypted = '';
          const currentEncrypted = encryptedRef.current;
          if (encryptionType === 'caesar') {
            decrypted = caesarDecrypt(currentEncrypted, parseInt(key) || 3);
          } else if (encryptionType === 'xor') {
            decrypted = xorEncrypt(currentEncrypted, key); // XOR is symmetric
          } else {
            decrypted = substitutionDecrypt(currentEncrypted);
          }
          ...
          if (success) {
            const newScore = Math.min(100, scoreRef.current + 20);
            setScore(newScore);
