...
export default function VPNTunnel({ onScoreChange }: Props) {
  ...
  // FIX: Replace useState initializer with useEffect for proper initialization
  useEffect(() => {
    initLevel();
  }, [initLevel]);

  // FIX: Cleanup timeout for showEncrypted on unmount
  useEffect(() => {
    if (!showEncrypted) return;
    const t = setTimeout(() => setShowEncrypted(false), 1500);
      ...
      case 'complete':
        {
          ...
          // FIX: Use functional update to avoid stale totalScore
          setTotalScore((prev) => {
  ...
  const handleSendPacket = () => {
    const newPacketsSent = packetsSent + 1;
    setPacketsSent(newPacketsSent);
    setShowEncrypted(true);
    // showEncrypted timeout is now handled by useEffect above
    if (newPacketsSent >= 3) {
      handleStep('complete');
    } else {
      setMessage(`Packet ${newPacketsSent} sent securely! ${3 - newPacketsSent} more to complete.`);
    }
  };
  ...
  const handleNextLevel = () => {
    if (currentLevel < VPN_LEVELS.length - 1) {
      ...
      // initLevel will be called by the useEffect when level changes
    }
  };
  ...
  const handleResetAll = () => {
    ...
    onScoreChange(0);
    // initLevel will be called by the useEffect
  };
